...
function getLikeMember(id){ // Nguoi ban thich
    //console.log(id);
    const requestOptions={
        method: 'GET',
        headers: authHeader()
...
function getLikeByUser(id){ // Nguoi thich ban
    const requestOptions={ 
        method: 'GET',
        headers: authHeader()
    }
    return fetch("/user/match/like/a/"+id,requestOptions).then(handleResponse);
}
