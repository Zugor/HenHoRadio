...
class LikeActivityListView extends React.Component{
    render(){
        const {like} =this.props;
        const result= (like && like.like && like.like.status && like.like.result ) ?  like.like.result : []
        return(
    ...
    <div className="tw3-box tw3-box--padding--off activityPage">
        <div className="tw3-row">
            ...
            { 
                result.map(item =>{
                    return (<LikeView item={item} key={item.id} />)
                })   
...
function mapStateToProps(state){
    const {authentication,like}=state;
    return {authentication,like};
}
const connected = connect(mapStateToProps)(LikeActivityListView);
export { connected as LikeActivityListView };